...
def generate_launch_description():
    # Kill processes on ports 8080 and 9090 before launch to avoid address in use error
    ...
    if not os.path.exists(web_ui_path):
        ...
        threading.Thread(
            target=lambda: subprocess.Popen(
                ['python3', '-m', 'http.server', '8080'], cwd=web_ui_path
            ),
            daemon=True
        ).start()
    ...
    nav_nodes = ['gnss_source_node', 'navigation_node']
