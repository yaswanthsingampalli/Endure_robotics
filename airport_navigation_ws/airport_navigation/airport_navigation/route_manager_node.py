...
class RouteManagerNode(Node):
    ...
    def handle_set_route(self, request, response):
        ...
        for idx, wp in enumerate(request.waypoints):
            try:
                # wp is geometry_msgs/Point with x=lon, y=lat, z=alt or vice versa
                lat = wp.y
                lon = wp.x
                alt = wp.z
                ...
                self.get_logger().info(f"✅ Waypoint {idx+1}: lat={lat}, lon={lon}, utm=({utm_x}, {utm_y})")
            except Exception as e:
                self.get_logger().error(f"❌ Failed to parse waypoint at index {idx}: {e}")
                response.success = False
                response.message = f"Invalid waypoint data at index {idx}"
                return response
