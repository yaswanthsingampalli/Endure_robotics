...
class RouteManagerNode(Node):
    def __init__(self):
        ...
        # Publish current target waypoint as PointStamped
        self.current_pub = self.create_publisher(PointStamped, '/current_target_position', 10)
        # Debug current waypoint as PointStamped
        self.debug_pub = self.create_publisher(PointStamped, '/current_waypoint', 10)
        # Publish all latlon waypoints (optional)
        self.latlon_pub = self.create_publisher(CustomWaypoint, '/waypoints_latlon', 10)
        # Publish all ENU waypoints as PointStamped
        self.enu_pub = self.create_publisher(PointStamped, '/waypoints_enu', 10)
        ...
        with self.lock:
            ...
            try:
                for idx, wp in enumerate(request.waypoints):
                    ...
                    self.route_enu.append((point_msg, name))
    ...
    def publish_current_target(self):
        if self.current_index < len(self.route_enu):
            point_msg, name = self.route_enu[self.current_index]
            self.current_pub.publish(point_msg)
            self.debug_pub.publish(point_msg)
            self.get_logger().info(f"📍 Published current target: {name}")
        else:
    ...
    def publish_all_enu(self):
        with self.lock:
            for point_msg, _ in self.route_enu:
                self.enu_pub.publish(point_msg)
