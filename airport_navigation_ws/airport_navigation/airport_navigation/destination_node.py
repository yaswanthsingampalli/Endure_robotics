...
class DestinationNode(Node):
    def __init__(self):
        ...
        # Store last published PointStamped message
        self.last_point_msg = None
        # Timer to periodically republish last destination
        self.timer = self.create_timer(1.0, self.publish_last_destination)
    ...
    def set_destination_callback(self, request, response):
        ...
        # Store last published PointStamped for republishing
        self.last_point_msg = point_msg
    ...
    def publish_last_destination(self):
        if self.last_point_msg is not None:
            # Update timestamp to current time
            self.last_point_msg.header.stamp = self.get_clock().now().to_msg()
            self.enu_publisher.publish(self.last_point_msg)
