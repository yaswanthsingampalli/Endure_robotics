...
class GNSSSourceNode(Node):
    def __init__(self):
        ...
        self.transformer = Transformer.from_crs("epsg:4326", "epsg:32614", always_xy=True)  # example UTM zone 14N
    ...
    def gnss_callback(self, msg: NavSatFix):
        ...
        if not self.origin_set:
            ...
            self.origin_pub.publish(origin_msg)          # publish once for current origin topic
            self.origin_fixed_msg = origin_msg           # store for repeated publishing
            self.get_logger().info(f"[GNSS] Origin set: lat={lat:.6f}, lon={lon:.6f}, alt={alt:.2f}")
